    ...
    <div class="pt-2" v-if="!loading" v-cloak>
        ...
        <transition name="fade">
            <div v-if="success" class="text-md text-center p-4 bg-green-100 text-green-700 shadow rounded">
                {{ chatterTrans.get('chatter.messages.Thank you, your replay was posted') }}.
            </div>
        </transition>
        ...
        <div v-if="!auth">
            <div class="text-md text-center p-4">{{ chatterTrans.get('chatter.messages.Please') }} <a class="font-bold text-blue-700" href="/login">{{ chatterTrans.get('chatter.messages.login') }}</a> {{ chatterTrans.get('chatter.messages.or') }} <a class="font-bold text-blue-700" href="/register">{{ chatterTrans.get('chatter.messages.signup') }}</a> {{ chatterTrans.get('chatter.messages.to leave a response') }}.</div>
        </div>
    </div>
</template>
